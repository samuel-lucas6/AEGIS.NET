...
internal sealed class AEGIS128Lx86 : IDisposable
{
    ...
    internal AEGIS128Lx86(ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
    {
    ...
    {
        if (_disposed) { throw new ObjectDisposedException(nameof(AEGIS128Lx86)); }
        int i = 0;
    ...
    {
        Vector128<byte> s0 = Aes.Encrypt(_s[7], _s[0] ^ m0);
        Vector128<byte> s1 = Aes.Encrypt(_s[0], _s[1]);
        Vector128<byte> s2 = Aes.Encrypt(_s[1], _s[2]);
        Vector128<byte> s3 = Aes.Encrypt(_s[2], _s[3]);
        Vector128<byte> s4 = Aes.Encrypt(_s[3], _s[4] ^ m1);
        Vector128<byte> s5 = Aes.Encrypt(_s[4], _s[5]);
        Vector128<byte> s6 = Aes.Encrypt(_s[5], _s[6]);
        Vector128<byte> s7 = Aes.Encrypt(_s[6], _s[7]);
