...
internal sealed class AEGIS256Arm : IDisposable
{
    ...
    internal AEGIS256Arm(ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
    {
    ...
    {
        if (_disposed) { throw new ObjectDisposedException(nameof(AEGIS256Arm)); }
        int i = 0;
    ...
    {
        Vector128<byte> s0 = Aes.MixColumns(Aes.Encrypt(_s[5], Vector128<byte>.Zero)) ^ _s[0] ^ message;
        ...
        Vector128<byte> s4 = Aes.MixColumns(Aes.Encrypt(_s[3], Vector128<byte>.Zero)) ^ _s[4];
